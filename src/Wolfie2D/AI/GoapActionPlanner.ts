...
export default class GoapActionPlanner<T extends GoapAction> {
    mapping: Map<number, T | string>;
    graph: Graph;
    path: Array<number>;

    plan(goal: string, possibleActions: Array<T>, currentStatus: Array<string>): Stack<T> {
		...
		while(this.path[i] !== -1){
            if (this.path[i] !== 0){
			    plan.push(<T>this.mapping.get(this.path[i]));
            }
			i = this.path[i];
		}
    ...
    buildTree(root: number, goal:string, possibleActions: Array<T>, currentStatus: Array<string>): void {
